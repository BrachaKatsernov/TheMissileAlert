    ...
    public class BLimp : IBL
    {
        ...
        public void UpdateAssessment(Assessment assessment)
        {
            try
            {
                ...
                //remove last assessments 
                foreach(Location_ l in assessment.Locations)
                {
                    dal.RemoveLocation(l.id);
                    assessment.Locations.Remove(l);
                }
                ...
                {
                    assessment.Locations.Add(new Location_(c.Latitude, c.Longitude));
                }
        ...
        public void AddFall(Fall fall)
        {
            try
            {
                ...
                List<Assessment> possible = (from a in GetAllAassessments()
                                            where a.start <= fall.time &&
                                            a.end >= fall.time
                                            select a).ToList();
                possible[0].Falls.Add(new Fall(fall.time,fall.image));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
